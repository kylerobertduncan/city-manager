...
function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (window.confirm("Error loading data from local storage. Would you like to reset the data?")) resetLocalStorage();
    }
  }, []);
  ...
  function resetLocalStorage() {
    if (!window.confirm("You are about to erase any saved data. Would you like to continue?")) return;
    // check for an existing item and if present, remove it
    if (localStorage.getItem(localStorageId)) localStorage.removeItem(localStorageId);
    ...
    <Grid container className="App">
      ...
      <Grid component="main" item xs={12} lg={9} sx={{ position:"relative" }}>
        ...
        <Box className="floatingElement" sx={{ position:"absolute", top: 0, right: 0 }}>
          Click: Lng: {featureLngLat.lng.toFixed(4)} | Lat: {featureLngLat.lat.toFixed(4)}
        </Box>
        
        {/* Toolbar */}
        <Stack alignItems="center" direction="row" paddingY="12px" spacing={2} sx={{ position:"absolute", bottom: 0, left: "50%", translate:"-50%" }}>
          ...
          <Button onClick={resetLocalStorage} variant="outlined">Clear Data</Button>
        </Stack>
